...
| `rights`          | required          | required          | A pointer to where to find access and usage rights information. For RMC material where information is held in the archival collection management system the value will be `archival_cms`, other values might be a `package_id`. We may want to add extra special values in the future. |
