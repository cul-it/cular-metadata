...
| `source_path`  | required          | not-allowed       | Location of package files in source data. This will be initially left blank by Dianne, and then populated using Shinwoo's preingest code. |
...
| `size`         | optional          | required          | Size of the file in bytes, an integer value. If not present for ingest, will be filled in before storage | `ingest_date`  | not-allowed       | required          | Date of ingest of the file. |
